...
namespace PTOX_LIB.HL7.Controller
{
	/// <summary>
	/// BuildPV1
	///     Validate and Build up the HL7 PV1 segment object
	/// </summary>
	public class BuildPV1 : HL7Parser
	{
		const string modName = "BuildPV1";
		...
		/// <summary>
		/// GetPV1
		///     Get the HL7 PV1 segment and parse into the PV1 object.
		...
		/// <returns>PV1 object</returns>
		public PV1 GetPV1(HL7Encoding _encode, string line)
		{
			const string fnName = "GetPV1";
			...
			try
			{
				pv1.SegmentMsg = line;
				pv1.Segment = "PV1";
				segError = Validate(pv1, _encode);

				// var enumCnt = Enum.GetNames(typeof(mshElements)).Length;
				foreach (int i in Enum.GetValues(typeof(Pv1Elements)))
				{
					nIdx = i;
					...
					if (obj == null)
					{
						...
						RequiredField rqFld = pv1.RequiredFields.Find(x => x.FieldName.Equals(sTmp1));
						if (rqFld != null && rqFld.IsRequired)
						{
							pv1.Errors.Add(string.Format("{0}:{1} - WARNING Element ({2}) not found in segment, at field {3}", modName, fnName, ((Pv1Elements)i).ToString(), i));
						}
						continue;
					}
					switch ((Pv1Elements)i)
					{
						case Pv1Elements.Segment:
							pv1.Segment = (string)obj;
							break;
						case Pv1Elements.SeqId:
							if (!string.IsNullOrEmpty((string)obj))
							{
								pv1.SegId = int.Parse((string)obj);
							}
							break;
						case Pv1Elements.ConsultingDr:
							pv1.ConsultingDr = (string)obj;
							break;
						case Pv1Elements.OrderingProvider:
							pv1.OrderingProvider = (string)obj;
							break;
						case Pv1Elements.ReferringDr:
							pv1.ReferringDr = (string)obj;
							break;

						default:
							pv1.Errors.Add(string.Format("{0}:{1} - Error element ({2}) not found", modName, fnName, ((Pv1Elements)i).ToString()));
							break;
					}
				}
			}
			...
			{
				string sErr = string.Format("Exception:{0}:{1} - ({2}) {3}", modName, fnName, ((Pv1Elements)nIdx).ToString(), ex);
				pv1.Errors.Add(sErr);
		...
		/// </summary>
		/// <param name="seg">PV1 object</param>
		/// <returns>list<SegmentError></returns>
		private List<SegmentError> Validate(PV1 seg, HL7Encoding _encode)
		{
